...
class Router(object):
	...
	def deliver_packet(self, packet):
		if random.random() > self.drop_probability:
			self.log.debug("Delivered packet from %d to %d: %s", packet.sender, packet.dest, packet.msg)
			self.clients[packet.dest].incoming.interrupt(packet)
		else:
			self.log.warning("Dropped packet from %d to %d: %s", packet.sender, packet.dest, packet.msg)