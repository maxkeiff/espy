...
class channel_noise_simulator:
    ...
    def randomise_bits_list_gauss_dist(self, bits, deviation):
        """A function to change the bits to floats modified with a gaussian distribution (with mean 0)
            ARGS: a list of bits, the deviation of the gauss distribution
            RETURN: a list of floats
        """

        new_signals = []
        for b in bits:
            new_signals.append((b + numpy.random.normal(0,deviation)))
